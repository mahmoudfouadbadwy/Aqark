...
class AddAdvertisementDataSource{
    ...
    func addFreeAdvertisement(){
        dataBaseRef.child("Users_Ads").child(userID).child("countOfFree").observeSingleEvent(of: .value) { (data) in
            if let value = data.value{
                if value is NSNull{
                    self.uploadeImageToStorage { (result) in
                        if result{
    ...
    func uploadeImageToStorage(compeltion : @escaping (Bool)->Void){
                ...
                self!.getImageUrl {[weak self] (value, myError) in
                    if myError == nil{
                        ...
                        if self?.images.count != self?.advertisement.images.count
                        {
                            ...
                            self?.uploadeImageToStorage(compeltion: { (reslult) in
