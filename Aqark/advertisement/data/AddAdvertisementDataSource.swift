...
class AddAdvertisementDataSource{
    ...
    func addFreeAdvertisement(){
        dataBaseRef.child("Users_Ads").child(userID).child("countOfFree").observeSingleEvent(of: .value) { (data) in
            if let value = data.value{
                if value is NSNull{
                    self.uploadeImageToStorage(compeltion: self.addFirstFreeAdvertisement)
                }else{
                    self.numberOfAdvertisement = value as! Int
                    if(self.numberOfAdvertisement < 20)
                    {
                        print("can upload")
                        self.uploadeImageToStorage(compeltion: self.addSecondFreeAdvertisement)
                    }else{
                        // go to payment page
    ...
    func uploadeImageToStorage(compeltion : @escaping ()->()){
