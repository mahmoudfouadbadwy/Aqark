        ...
        for id in arrOfIDs
        {
            ...
            ref.child("Advertisements").child(id).observeSingleEvent(of: .value) { (snapshot) in
                if (snapshot.exists())
                {
                    ...
                    let advertisement:ProfileAdvertisement = ProfileAdvertisement(propertyType:propertyType,price:Double(price) ?? 0.0,address:location,bed:bedrooms,bathroom:bathrooms,propertySize:size, propertyImage: images[0], paymentType: payment, advertisementType: advertisementType, advertisementId: id)
